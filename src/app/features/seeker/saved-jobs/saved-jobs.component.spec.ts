...
describe('SavedJobsComponent', () => {
  ...
  const mockAuthService = {
    currentUser: signal({ id: 5, role: 'JOB_SEEKER', name: 'J', email: 'j@t.com', token: 't' }),
  ...
  beforeEach(async () => {
    try { spyOn(Swal, 'fire').and.returnValue(Promise.resolve() as any); } catch (e) { }
  ...
  xit('should handle error gracefully when loading saved jobs', () => {
  ...
  describe('unsaveJob()', () => {
    it('should remove job from savedJobs list after unsave', fakeAsync(() => {
      ...
      tick(5000);
      try { flush(); discardPeriodicTasks(); } catch (e) { }
