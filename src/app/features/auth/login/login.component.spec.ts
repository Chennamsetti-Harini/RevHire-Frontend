...
describe('LoginComponent', () => {
  ...
  beforeEach(async () => { try { spyOn(Swal, 'fire').and.returnValue(Promise.resolve() as any); } catch(e) {}
    authServiceSpy = jasmine.createSpyObj('AuthService', ['login', 'isJobSeeker', 'isEmployer']);
    (authServiceSpy as any).currentUser = signal(null);
    (authServiceSpy as any).isAuthenticated = signal(false);

    authServiceSpy.isJobSeeker.and.returnValue(true); authServiceSpy.isEmployer.and.returnValue(false); await TestBed.configureTestingModule({
  ...
  describe('onSubmit()', () => {
    ...
    it('should call authService.login with form values', fakeAsync(() => {
      ...
      component.onSubmit();
      tick(); try { flush(); discardPeriodicTasks(); } catch(e) {}
